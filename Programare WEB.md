...
**55. Care metode HTTP nu presupun trimiterea de conținut în cerere?**
...
**56. Care antete sunt obligatorii pentru o cerere HTTP/1.1?**
...
**57. Care informații despre un URL accesat prin POST sunt adevărate?**
...
**58. Serverul răspunde cu `404 Not Found`. Ce este adevărat?**
...
**59. Un URL de pe back-end poate fi invocat atât prin GET cât și prin POST?**

- Doar dacă datele primite sunt aceleași
- Nu
- ✅ **Da**

**60. Pe ce nivel al stivei TCP/IP se situează protocolul HTTP?**
...
**61. Care este un avantaj al metodei GET față de POST?**
...
**62. Cărui fișier i se trimit datele la submit?**
...
**63. Care este un query string posibil la submit?**
...
**64. Cum este posibil să existe 2 butoane de submit într-un formular?**
...
**65. Cum se salvează date (nume și vârstă) folosind AJAX prin GET?**

- ✅ `$.get("save.php?nume=Ion&varsta=10")`
- ✅ `$.get("save.php", { nume: 'Ion', varsta: 10 })`
- ✅ `$.get("save.php", 'nume=Ion&varsta=10')`

**66. Un apel AJAX se poate face către o resursă statică?**

- Doar dacă se trimit și parametri
- Nu, niciodată
- ✅ **Da, întotdeauna**

**67. În ce condiții metoda `send` se apelează cu șirul vid?**

- Dacă request-ul AJAX se face prin HTTP
- Metoda send nu poate fi apelată cu șirul vid
- ✅ Dacă request-ul AJAX se face prin **GET**

**68. Care metode jQuery folosesc AJAX?**
...
**69. Ordonați de la cea mai simplă la cea mai configurabilă:**

- ✅ `load` → `$.get` → `$.ajax`

**70. Pentru a verifica succesul unui apel AJAX:**
...
**73. Funcția `onreadystatechange` se apelează:**
...
**74. Răspunsul sosit printr-un apel AJAX poate fi:**
...
**75. Care dintre următoarele sunt scriptlet-uri valide în PHP?**
...
**76. Un fișier PHP poate fi executat:**
...
**77. Cum se poate interschimba valoarea variabilelor `$a` și `$b` în PHP?**

- ✅ `list($a, $b) = [$b, $a];`
- ✅ `list($a, $b) = array($b, $a);`

**78. În PHP, `return` este folosit pentru:**
...
**79. Ce face `session_start()` în PHP?**
...
**80. Cum se concatenează două șiruri în PHP?**
...
**84. Cum se introduc comentarii într-un scriptlet PHP?**
...
**85. Cum se obțin parametrii la apăsarea ancorei `<a href="www.siteulmeu.com?addFav=1">`?**
...
**86. Conectarea la o bază de date într-o aplicație web Java se recomandă:**
...
**87. Parametrii de configurare sunt preluați în cadrul metodei:**

- `applicationCreated()`
- `init()`
- ✅ `contextInitialized()`

**88. O bază de date embedded se recomandă a fi memorată:**

- Oriunde altundeva în sistemul de fișiere
- În `app/databases/`
- ✅ În folderul `WEB-INF` (inaccesibil clienților web)

**89. În câte instanțe se instanțiază un servlet?**

- O instanță separată la fiecare cerere
- În nicio instanță
- ✅ **O singură instanță**

**90. O sesiune într-o aplicație web Java se termină cu:**
...
**91. Injecțiile JavaScript se datorează:**
...
**92. Cum se poate „fura" un cookie de sesiune?**
...
**93. Pentru evitarea injecțiilor SQL în PHP 7 se recomandă:**

- Eliminarea ghilimelelor și apostrofelor
- `mysql_real_escape_string`
- ✅ Folosirea de **prepared statement**-uri

**94. Măsuri pentru evitarea injecțiilor SQL:**
...
**95. Măsuri pentru evitarea vulnerabilităților de tip XSS:**
...
**96. Care funcție PHP este folosită pentru a preveni XSS?**

- `mysql_real_escape_string`
- `filter_xss`
- ✅ `htmlentities`

**97. Care vulnerabilități pot fi exploatate pentru a fura sesiunea unui utilizator autentificat?**

- SQL Injection
- Cross-Site Request Forgery (CSRF)
- ✅ Cross-Site Scripting (XSS)

**98. Care expresii jQuery se pot folosi interschimbabil?** *(variantă examen)*
